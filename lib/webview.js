...
const {span, ul, li, button, i, td, tr, th, table, code, div, label, input, hr, h1, h4, makeDOMDriver} = require('@cycle/dom')
...
let behaviourView = level => behaviour => [tr(`.node`, {
  dataset: { index: behaviour.index.toString() },
  class: {
    historic: behaviour.in_history,
    active: behaviour.active,
    hide: !behaviour.active,
    branching: behaviour.branching,
    leaf: behaviour.children.length == 0,
    branch: behaviour.children.length > 0
  }
}, [
  td(".index", level + behaviour.index + (behaviour.children.length > 0 ? "." : "")),
  td(".status", [behaviour.success ? "✓" : ""]),
  td(".deltaC", behaviour.deltaC || ""),
  ])
  ].concat(behaviour.children.filter(b => b.active).map(behaviourView( level + behaviour.index + "." )).reduce((a, e) => a.concat(e), []))
  ...
  const vdom$ = onion.state$
    .map(state => {
      if(!state.path || state.path.length == 0 || !state.config) {
        ...
        let storage_str = table(".storage", Object
          .keys(storage_object)
          .map(key => tr([td(".key", key), td(".value", storage_object[key])])))
        ...
        return div([
          ...
          div(".player", [
            ...
            table(".behaviour", behaviourView("")(behaviour))
          ])
        ])
      }
    });
