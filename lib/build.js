...
const intwords = {
    "int128" : name => `#rangeUInt(128, ${ name.replace('#signed','')})`,
    "int256" : name => `#rangeUInt(256, ${ name.replace('#signed','')})`
}
...
const make_abi_dsl = ({type, name}, i) =>
      "#" + type +
      ((type == "int256" || type == "int128") ? "(#signed(ABI_" + name + "))" : "(ABI_" + name + ")")
  ...
  let act = act_arr
    .reduce((a, e) => {
      ...
      if(/^behaviour\s/.test(head)) {
        ...
        if(!silent) console.log("parse " + a.subject + "." + a.name)
        ...
        const _interface_types = interface
          .reduce((a, [v, k]) => {
              a["ABI_" + k] = (v == "int256" || v == "int128")
                  ? longForm(v.replace("int","uint"))
                  : longForm(v);
            return a;
          }, {})
  ...
  const getTerm = ({
    ...
    const buildAccount = varname => {
        let storage_pass = Object.keys(act.storage[varname])
            .reduce((a, key) => `addNew(${a}, ${storagekeys(realname(varname), key)}, (${clean(act.storage[varname][key][0])} => ${clean(act.storage[varname][key][1])}))`, `${varname}_STORE`)
        let storage_fail = Object.keys(act.storage[varname])
            .reduce((a, key) => `addNew(${a}, ${storagekeys(realname(varname), key)}, ${clean(act.storage[varname][key][0])})`, `${varname}_STORE`)
        let storage = (pass && !oog) ? storage_pass : (storage_fail + '=> _')
...
/*    .concat(
        Object.keys(act.types)
            .filter(name =>
                    {
                        console.log(act.types[name])
                        console.log(intwords[act.types[name]])
                        return (act.types[name] == "int256" || act.types[name] == "int128" )
                        })
            .map(name => intwords[act.types[name]](name))
    )*/
    //assert storage entries have distinct keys from abstract STORE
    .concat(flatten(Object.keys(act.storage)
      .map(varname => Object.keys(act.storage[varname])
           .map(key => `notBool #inKeys(${varname}_STORE, ${storagekeys(realname(varname), key)})`)
      )
    ))
    .concat(act.if || [])
